	...
	xmlHttp.onload = function() { 
		if (xmlHttp.readyState === 4){
			if (xmlHttp.status === 200) {
				console.log('HTTP fetch successful; you may now dox me.');
...
function socialPost(post, header, notice_text, headerId, contentId){
	if(post.cw){
		...
		header += "something";
		if(post.attachments) header += " with media"
		var content = text_truncate(post.content, 300);
	}
	var link = post.link;

	document.getElementById(headerId).innerHTML = header + '<i class="less-important"> via ' + notice_text + "</i>";
	document.getElementById(contentId).innerHTML = "<a href=\"" + link + "\">" + content + "</a>";
...
function status_update(status){
	...
	document.getElementById("music-header").innerHTML = message1;
	document.getElementById("music-body").innerHTML = message2;
	...
	if(media.type == 'episode'){
		...
		// IMDb
		var show_link = "https://libremdb.iket.me/title/" + media.imdb_id
		var ep_link   = "https://libremdb.iket.me/title/" + media.ep.imdb_id
		// Fuck you Amazon.
		...
		if(media.weebshit) document.getElementById("tv-header").innerHTML = message1 + " weeb shit";
		else document.getElementById("tv-header").innerHTML = message1 + " a TV show";
		document.getElementById("tv-content").innerHTML = message2;
	} else{
		// movie

		// IMDb
		// var link = "https://www.imdb.com/title/" + media.imdb_id
		var link = "https://libremdb.iket.me/title/" + media.imdb_id
		// Fuck you Amazon again.
		...
		document.getElementById("tv-header").innerHTML = message1 + " a movie";
		document.getElementById("tv-content").innerHTML = message2;
	}
	...
	switch(status.media.latest){
		case "tv":
			...
			changeHeaderType("music-header", "h3")
	...
	socialPost(post, idontfuckingknow, notice_text, "soc-header", "soc-body")
	...
	// if(isTimestampOver14DaysAgo(status.social.mstdn.posted))
	//  	document.getElementById("social-alt").style.display = "none";
}
...
function fuck(shit){
	...
	try {
		...
		document.getElementById("music-body").innerHTML = "I am not currently listening to anything right now; I'll show you my most recently listened song, this won't take too long!";
...
function convertSecondsToMinutesAndSeconds(seconds) {
	...
	var remainingSeconds = Math.floor(seconds % 60); // Calculate remaining seconds
	// uh buddy, here's an idea: seconds = Math.floor(seconds)???????
...
//         document.getElementById("music-header").innerHTML = "am connecting to ListenBrainz";
//         document.getElementById("music-body").innerHTML = "since I am not watching anything on Trakt, I am going to show you what I'm listening to. just give me a second...";
...
//         document.getElementById("music-header").innerHTML = "am watching a TV show";
//         document.getElementById("music-body").innerHTML = message2;
//     } else{
//         // movie
//         var movie = data.movies[0];
//         var message2 = "<strong>" + movie.title + "</strong><br><em>" + movie.year + "</em>";
//         document.getElementById("music-header").innerHTML = "am watching a movie";
//         document.getElementById("music-body").innerHTML = message2;
//     }
// }
...
function toothole(toots){  // toothole because funny or something idrk
	...
	document.getElementById("soc-header").innerHTML = idontfuckingknow;
	document.getElementById("soc-body").innerHTML = content;
}
